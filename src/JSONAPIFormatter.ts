  ...
  async middleware(ctx: BambusContext<T>, next: Function){
    ...
    if(ctx.model){
      ...
      if(!this.serializers[toInclude]){
        let opts = {attributes: getAllPresentableAttributesFor(this.modelClass)};
        addRelationships(this.modelClass, opts, toInclude)
        this.serializers[toInclude] = new JSONAPISerializer(this.name + "s", opts);
      }
...
function addRelationships<T extends Model<T>>(modelClass: ModelClazz<T>, opts: IOptions, include: String){
